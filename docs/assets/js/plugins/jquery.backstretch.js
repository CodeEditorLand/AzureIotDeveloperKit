...
(($, window, undefined) => {
	...
	var optimalSizeImages = (() => {
		...
		var widthInsertSort = (arr) => {
			for (var i = 1; i < arr.length; i++) {
				...
				while (
					arr[j - 1] &&
					Number.parseInt(arr[j - 1].width, 10) >
						Number.parseInt(tmp.width, 10)
				) {
					arr[j] = arr[j - 1];
					--j;
				}
		...
		var selectBest = (containerWidth, containerHeight, imageSizes) => {
			...
			for (var j = 0, image; j < imageSizes.length; j++) {
				...
				if (
					...
					Number.parseFloat(image.pixelRatio) !== devicePixelRatio
				) {
	...
	var processAlignOptions = (options, required) => {
		if (options.alignX === "left") {
			...
			options.alignX = Number.parseFloat(options.alignX);
		...
		if (options.alignY === "top") {
			...
			options.alignY = Number.parseFloat(options.alignY);
