...
(function ($, window, undefined) {
	...
	var sidebarFix = function () {
		...
		if (window.innerWidth >= 1024) {
			...
			if (
				articleTop < $(this).scrollTop() + menuHeight &&
				sidebarBottom > $(window).height() - 30
			) {
				$(".sidebar__right_fix").css(
					"height",
					$(window).height() -
	...
	var trackClickNumber = function (element) {
		...
		try {
			dataLayer.push({
				...
				"gtm.element": element,
				"gtm.elementUrl": url,
				"gtm.elementClass": $(element).attr("class")
					? $(element).attr("class")
					: "",
				"gtm.elementId": $(element).attr("id")
					? $(element).attr("id")
					: "",
				"gtm.elementTarget": $(element).attr("target")
					? $(element).attr("target")
					: "",
			});
		} catch (e) {
